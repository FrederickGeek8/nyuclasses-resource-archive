...
class Client(object):
    ...
    def list(self, remote_path=root, get_info=False, recursive=False):
        ...
        remote_resource_names = [
            urn.filename()
            for urn in urns
            if Urn.compare_path(path, urn.path()) is False
        ]

        # # FIXME: this client seems to try and follow those links
        # remote_resource_names = list(
        #     filter(lambda r: not r.endswith(".URL"), remote_resource_names)
        # )

        return remote_resource_names
    ...
    def pull(self, remote_directory, local_directory):
        ...
        pbar = tqdm(remote_resource_names)
        for remote_resource_name in pbar:
            pbar.set_description(remote_resource_name)
            ...
            if remote_urn.path().endswith("/"):
                ...
                if (
                    ...
                    and self.is_more_recent_and_same_size(local_path, remote_path)
                ):
                    continue
    ...
    def is_more_recent_and_same_size(self, local_path, remote_path):
        try:
            remote_info = self.info(remote_path)
            remote_size = int(remote_info["size"])
            local_size = int(os.stat(local_path).st_size)
            ...
            return (
                ...
                and remote_last_mod_date_unix_ts < local_last_mod_date_unix_ts
            )
    ...
    def does_local_size_match(self, local_path, remote_path):
        try:
            remote_info = self.info(remote_path)
